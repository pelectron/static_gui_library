...
// Let's do the same for now.
  ...
  constexpr uint64_t umul128(const uint64_t a, const uint64_t b, uint64_t* const productHi) {
  ...
  constexpr uint64_t shiftright128(const uint64_t lo, const uint64_t hi, const uint32_t dist) {
  ...
  constexpr uint64_t umulh(const uint64_t a, const uint64_t b) {
  ...
  constexpr uint64_t div5(const uint64_t x) { return umulh(x, 0xCCCCCCCCCCCCCCCDu) >> 2; }
  ...
  constexpr uint64_t div5(const uint64_t x) { return x / 5; }

  constexpr uint64_t div10(const uint64_t x) { return x / 10; }

  constexpr uint64_t div100(const uint64_t x) { return x / 100; }

  constexpr uint64_t div1e8(const uint64_t x) { return x / 100000000; }

  constexpr uint64_t div1e9(const uint64_t x) { return x / 1000000000; }

  constexpr uint32_t mod1e9(const uint64_t x) { return (uint32_t)(x - 1000000000 * div1e9(x)); }

#endif // defined(RYU_32_BIT_PLATFORM)

  constexpr uint32_t pow5Factor(uint64_t value) {
  ...
  constexpr bool multipleOfPowerOf5(const uint64_t value, const uint32_t p) {
    // I tried a case distinction on p, but there was no performance difference.
    return pow5Factor(value) >= p;
  }

  // Returns true if value is divisible by 2^p.
  constexpr bool multipleOfPowerOf2(const uint64_t value, const uint32_t p) {
    // assert(value != 0);
    // assert(p < 64);
...
//       results are not perfectly aligned.
  ...
  constexpr uint64_t mulShiftAll64(uint64_t              m,
                                   const uint64_t* const mul,
                                   const int32_t         j,
                                   uint64_t* const       vp,
                                   uint64_t* const       vm,
