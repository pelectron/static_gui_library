...
namespace ryu {
  namespace cx {
    ...
    constexpr ryu::Status s2f_n(const char* buffer, const int len, float* result) noexcept {
  ...
  constexpr ryu::Status s2f_n(const char*         buffer,
    ...
    if (e10 >= 0) {
      ...
      e2 = floor_log2_func(m10) + e10 + ryu::log2pow5(e10) - (FLOAT_MANTISSA_BITS + 1);

      // We now compute [m10 * 10^e10 / 2^e2] = [m10 * 5^e10 / 2^(e2-e10)].
      // To that end, we use the FLOAT_POW5_SPLIT table.
      int j = e2 - e10 - ryu::ceil_log2pow5(e10) + ryu::detail::float_pow5_bitcount;
      // assert(j >= 0);
      ...
      // We now compute [m10 * 10^e10 / 2^e2] = [m10 / (5^(-e10) 2^(e2-e10))].
      int j = e2 - e10 + ceil_log2pow5(-e10) - 1 + ryu::detail::float_pow5_inv_bitcount;
  ...
  inline Status s2f_n(const char* buffer, const int len, float* result) noexcept {
    return ryu::s2f_n(buffer, len, result, &ryu::floor_log2, &ryu::int32Bits2Float);
  }
} // namespace ryu
