...
namespace sgl {
  ...
  template <size_t Len, char c, char... chars>
  constexpr size_t to_decimal() {
    return pow10(Len) * (c - '0') + to_decimal<Len - 1, chars...>();
  }
  template <char c>
  constexpr int sign() {
    return c == '-' ? -1 : 1;
  ...
  constexpr float pow10f(int exp) {
    float f{1.0f};
    if (exp < 0) {
      exp = -exp;
      while (exp--) {
  ...
  constexpr cx_arg<T, sizeof...(chars)> make_cx_arg() {
    ...
    if constexpr (num_dots<chars...>() == 0) {
      ...
      return static_cast<T>(decimal) * pow10f(exponent);
    } else {
      if constexpr (num_es<chars...>() == 0) {
        ...
        res = static_cast<T>(decimal) + static_cast<T>(fractional) / pow10f(num_frac_digits);
      } else {
        // [+-]XXX.YYYe[+-]ZZZ
        static_assert(num_dots<chars...>() != 0, "");
        constexpr int    decimal = to_signed_first_n<dot_index, chars...>();
        ...
        constexpr int    fractional = ::sgl::fractional<dot_index, e_index, chars...>();
        res = (static_cast<T>(decimal) + static_cast<T>(fractional) / pow10f(num_frac_digits)) *
              pow10f(exponent);
      }
    }
  ...
  constexpr cx_arg<float, sizeof...(chars)> operator"" _float() {
    return make_cx_arg<float, chars...>();
  }
