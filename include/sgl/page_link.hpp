...
namespace sgl {
  ...
  /**
    A page link is a button item, that, when clicked, causes the menu to switch to the page
    specified by the link.

    To create a PageLink, there are two ways:
    ...
    @tparam Name an instance of sgl::Name.
    @tparam TextSize text size of the link
    @tparam CharT character type of the pagelink
   */
  ...
  public:
    ...
    /**
      create a page link with the @ref sgl::Name "name" of the page it should link to. The
    ...
    /**
      create page link with page name to link to and text to show
    ...
    /**
      static overload of ItemBase::set_menu.
      The menu type cannot be a part of the template parameters given to a page link on
    ...
    /**
      get name of the page this links to.
  ...
  /**
    ...
    @{
    create a pagelink item with only the name of the page to link to.
  ...
  /**
    create a pagelink with name and text literal. Name, Character type and textsize are
  ...
  /**
    create pagelink by explicitly specifying the textsize. The rest of the template
    arguments are deduced.
    ```cpp
    auto link = pagelink<40>(SGL_NAME("a name"), "displayed text"_sv);
    ```
   *
    @tparam TextSize text size of pagelink
