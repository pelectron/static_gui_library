 ...
 * This file contains the sgl::Page class template and the page iteration functions
...
namespace sgl {
  ...
  /**
    This class represents a page of a menu. A **page** is a flat container of **items**.
    It is a non recursive data structure, i.e. a page cannot contain subpages as items, see
    sgl::PageLink for that. A page is responsible for navigating through the items and delegating
    user input the the current item.

    A page consists of the following data:

     - a @ref NamedTuple "named tuple" of *items*.
     - an *input handler*. It manages how the page gets navigated and how user input is passed on
    to its items.
     - an enter and exit *action*. These will be called by the menu when a page is entered, i.e.
    becomes the current page, or exited, i.e. another page becomes current. By default these do
    nothing.
    ...
    The *items*, *input handler*, and *page actions* are all concepts described
    [here](markdown/concepts.md) in detail.

    For more information about input handling see [here](markdown/input_handling.md).
    ...
    Like the sgl::NamedTuple, the page also offers possibilities to apply a functor on its items
    with overloads for sgl::for_each(), and Page::for_each_item(). Additionally, any functor that
  ...
  public:
    ...
    /**
      Concrete input handler type. A page input handler is a callable with a call signature
    ...
    /**
      Set the menu for items which need it, for example sgl::PageLink.
    ...
    /**
      apply f on all items in the page with the items' **declared
      type**.
    ...
    /**
      apply f on each item in page with the items name. See NamedTuple::for_each_with_name
    ...
    /**
      apply f on the current item. An example is below.
    ...
    /**
      get the name of the i-th item as a string_view. Returns an empty string_view if i is
      out of range.
      @param i item index
    ...
    /**
      get the text of the i-th item as a string_view. Returns an empty string_view if i is
      out of range.
      @param i item index
  ...
  private:
    ...
    sgl::input     start_edit_{sgl::input::enter};  ///< page will switch to edit mode if input if
                                                    ///< equal to start_edit_ is given
    sgl::input stop_edit_{sgl::input::enter}; ///< page will switch to navigation mode if input is
                                              ///< equal to stop_edit_.
  ...
  /**
    apply f on each item in a page. This will call f with a reference to the item for each
    item.
  ...
  /**
    apply f on each item in a page. This will call f with a const reference to the item for
    each item.
    @tparam NameList name list of page
    @tparam ItemList type list of page
    @tparam F functor type
  ...
  /**
    apply f on each item in a page. f will be called with the name of the item as an
  ...
  /**
    apply functor f on the current item of a page. This will call f with a reference to the
  ...
  /**
    apply functor f on the current item of a page. This will call f with a const reference
