...
namespace sgl {
  ...
  template <typename CharT, size_t Capacity>
  constexpr static_string<CharT, Capacity>::static_string(const CharT* str, size_t size) noexcept {
    overwrite(str, size);
  }

  template <typename CharT, size_t Capacity>
  constexpr static_string<CharT, Capacity>::operator string_view<CharT>() const noexcept {
    return string_view<CharT>{data_, size_};
  }

  template <typename CharT, size_t Capacity>
  constexpr size_t static_string<CharT, Capacity>::size() const noexcept {
    return size_;
  }

  template <typename CharT, size_t Capacity>
  [[nodiscard]] constexpr size_t static_string<CharT, Capacity>::capacity() const noexcept {
    return Capacity;
  }

  template <typename CharT, size_t Capacity>
  [[nodiscard]] constexpr CharT* static_string<CharT, Capacity>::c_str() noexcept {
    return data_;
  }

  template <typename CharT, size_t Capacity>
  [[nodiscard]] constexpr const CharT* static_string<CharT, Capacity>::c_str() const noexcept {
    return data_;
  }

  template <typename CharT, size_t Capacity>
  [[nodiscard]] constexpr CharT* static_string<CharT, Capacity>::data() noexcept {
    return data_;
  }

  template <typename CharT, size_t Capacity>
  [[nodiscard]] constexpr const CharT* static_string<CharT, Capacity>::data() const noexcept {
    return data_;
  }

  template <typename CharT, size_t Capacity>
  [[nodiscard]] constexpr CharT* static_string<CharT, Capacity>::begin() noexcept {
    return data_;
  }

  template <typename CharT, size_t Capacity>
  [[nodiscard]] constexpr const CharT* static_string<CharT, Capacity>::begin() const noexcept {
    return data_;
  }

  template <typename CharT, size_t Capacity>
  [[nodiscard]] constexpr const CharT* static_string<CharT, Capacity>::end() const noexcept {
    return data_ + size_;
  }
  ...
  template <typename CharT, size_t Capacity>
  constexpr void static_string<CharT, Capacity>::append(string_view<CharT> str) noexcept {
  ...
  template <typename CharT, size_t Capacity>
  [[nodiscard]] constexpr bool static_string<CharT, Capacity>::is_empty() const noexcept {
    return size_ == 0;
  }

  template <typename CharT, size_t Capacity>
  constexpr static_string<CharT, Capacity>&
      static_string<CharT, Capacity>::operator=(const static_string& other) noexcept {
    if (this == &other) {
      return *this;
    }
  ...
  template <typename CharT, size_t Capacity>
  [[nodiscard]] constexpr CharT& static_string<CharT, Capacity>::operator[](size_t i) noexcept {
    return data_[i];
  }

  template <typename CharT, size_t Capacity>
  [[nodiscard]] constexpr const CharT&
      static_string<CharT, Capacity>::operator[](size_t i) const noexcept {
    return data_[i];
  }
