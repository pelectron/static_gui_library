...
namespace sgl {
  ...
  namespace tuple_detail {
    ...
    struct type_for<Name, NamedTuple<type_list<Names...>, type_list<Ts...>>> {
  ...
  constexpr NamedTuple<type_list<Names...>, type_list<Ts...>>::NamedTuple(
      const Ts&... elems) noexcept(nothrow_copy_constructible)
      : NamedValue<Names, Ts>{Names{}, elems}... {}

  template <typename... Names, typename... Ts>
  constexpr NamedTuple<type_list<Names...>, type_list<Ts...>>::NamedTuple(Ts&&... elems) noexcept(
  ...
  template <typename... Names, typename... Ts>
  template <typename Name>
  constexpr auto& NamedTuple<type_list<Names...>, type_list<Ts...>>::get(Name name) noexcept {
    if constexpr (!sgl::contains_v<Name, name_list_t>) {
      static_assert(sgl::contains_v<Name, name_list_t>, "No such name in tuple");
    } else {
  ...
  template <typename... Names, typename... Ts>
  template <size_t I>
  constexpr auto& NamedTuple<type_list<Names...>, type_list<Ts...>>::get() noexcept {
  ...
  template <typename... Names, typename... Ts>
  template <size_t I>
  constexpr const auto& NamedTuple<type_list<Names...>, type_list<Ts...>>::get() const noexcept {
    if constexpr (I >= sizeof...(Ts)) {
      static_assert(tuple_detail::always_false<I>, "Index out of range.");
    } else {
      using Name = type_at_t<I, name_list_t>;
  ...
  template <typename Name>
  constexpr const auto&
      NamedTuple<type_list<Names...>, type_list<Ts...>>::operator[](Name name) const noexcept {
    if constexpr (!sgl::contains_v<Name, name_list_t>) {
      static_assert(sgl::contains_v<Name, name_list_t>, "No such name in tuple");
    } else {
      return static_cast<const NamedValue<Name, tuple_detail::type_for_t<Name, This_t>>*>(this)
  ...
  template <typename... Names, typename... Ts>
  template <typename F>
  constexpr void NamedTuple<type_list<Names...>, type_list<Ts...>>::for_each(F&& f) noexcept(
      nothrow_invocable_for_each<F, Ts&...>) {
  ...
  template <typename... Names, typename... Ts>
  template <typename F>
  constexpr void NamedTuple<type_list<Names...>, type_list<Ts...>>::for_each(F&& f) const
  ...
  template <typename... Names, typename... Ts>
  template <typename F>
  constexpr void NamedTuple<type_list<Names...>, type_list<Ts...>>::for_each_with_name(F&& f) const
  ...
  constexpr void for_each(NamedTuple<NameList, TypeList>& tuple, F&& f) noexcept(
    ...
    return tuple.for_each(std::forward<F>(f));
  }
