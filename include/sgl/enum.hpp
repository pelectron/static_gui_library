...
namespace sgl {
  ...
  /// \brief create an enum item with every template parameter deduced. The signature is a bit
  ...
  /// \tparam EnumStringArgs rest of enum value/string pairs
  ...
  /// \brief create an enum item with every template parameter deduced and custom start index. See
  /// sgl::make_enum(const E& enum_val, const Str& enum_string, const EnumStringArgs&... args) for
  /// an explanation of the parameters after the start index. \code enum class Setting { opt1, opt2,
  /// opt3 };
  ...
  /// \brief create an enum item with every template parameter deduced and custom start index and
  ...
  /// \endcode
  ///
  /// \tparam InputHandler enum item input handler
  /// \tparam E the enum type
  /// \tparam Str string type
  /// \tparam EnumStringArgs rest of enum value/string pairs
