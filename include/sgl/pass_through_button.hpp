...
namespace sgl {
  /**
   * @brief This class can be inherited from if an item needs a button like behaviour. For an
  ...
  public:
    /// @brief base class of this item
    using Base = sgl::ItemBase<ItemImpl, Traits>;

    /// @brief concrete item type
    using item_type = typename Base::item_type;

    /// @brief string_View type of this item
    using StringView = typename Base::StringView;

    /// @brief concrete type used for storing click handlers.
